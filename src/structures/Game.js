...
class Game extends EventEmitter {
  ...
  startStreaming() {
    const emitor = this.client.api.stream(`/api/${this.client.type}/game/stream/${this.id}`);
    emitor.on("data", data => {
      // console.log(data)
      let state;
      switch (data.type) {
  ...
  async playMove(move) {
    ...
    if (played) {
      const data = await this.client.api.post(`/api/${this.client.type}/game/${this.id}/move/${played.from}${played.to}${played.promotion || ""}`);
